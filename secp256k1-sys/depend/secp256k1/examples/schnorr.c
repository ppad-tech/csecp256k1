...
int main(void) {
    ...
    while (1) {
        ...
        if (haskellsecp256k1_v0_1_0_keypair_create(ctx, &keypair, seckey)) {
            break;
        }
    }
     ...
     * `haskellsecp256k1_v0_1_0_keypair_xonly_pub` supports returning the parity for
     * other use cases such as tests or verifying Taproot tweaks.
     * This should never fail with a valid context and public key. */
    return_val = haskellsecp256k1_v0_1_0_keypair_xonly_pub(ctx, &pubkey, NULL, &keypair);
    assert(return_val);

    /* Serialize the public key. Should always return 1 for a valid public key. */
    return_val = haskellsecp256k1_v0_1_0_xonly_pubkey_serialize(ctx, serialized_pubkey, &pubkey);
    assert(return_val);
     ...
     *
     * We use haskellsecp256k1_v0_1_0_tagged_sha256 to create this hash. This function expects
    ...
    return_val = haskellsecp256k1_v0_1_0_tagged_sha256(ctx, msg_hash, tag, sizeof(tag), msg, sizeof(msg));
    assert(return_val);
     ...
     *
     * We use the haskellsecp256k1_v0_1_0_schnorrsig_sign32 function that provides a simple
    ...
    return_val = haskellsecp256k1_v0_1_0_schnorrsig_sign32(ctx, signature, msg_hash, &keypair, auxiliary_rand);
    assert(return_val);
     ...
     * be parsed correctly */
    if (!haskellsecp256k1_v0_1_0_xonly_pubkey_parse(ctx, &pubkey, serialized_pubkey)) {
    ...
    is_signature_valid = haskellsecp256k1_v0_1_0_schnorrsig_verify(ctx, signature, msg_hash, 32, &pubkey);
    ...
    /* Bonus example: if all we need is signature verification (and no key
       ...
       haskellsecp256k1_v0_1_0_context_create(). We can simply use the static (i.e., global)
       context haskellsecp256k1_v0_1_0_context_static. See its description in
       include/secp256k1.h for details. */
    is_signature_valid2 = haskellsecp256k1_v0_1_0_schnorrsig_verify(haskellsecp256k1_v0_1_0_context_static,
