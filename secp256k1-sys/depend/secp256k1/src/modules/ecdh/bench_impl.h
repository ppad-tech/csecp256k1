...
static void bench_ecdh_setup(void* arg) {
    ...
    CHECK(haskellsecp256k1_v0_1_0_ec_pubkey_parse(data->ctx, &data->point, point, sizeof(point)) == 1);
}
...
static void bench_ecdh(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        CHECK(haskellsecp256k1_v0_1_0_ecdh(data->ctx, res, &data->point, data->scalar, NULL, NULL) == 1);
    }
}
...
static void run_ecdh_bench(int iters, int argc, char** argv) {
    ...
    data.ctx = haskellsecp256k1_v0_1_0_context_create(SECP256K1_FLAGS_TYPE_CONTEXT);
