...
typedef struct {
    ...
    const haskellsecp256k1_v0_1_0_keypair **keypairs;
...
static void bench_schnorrsig_sign(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        msg[0] = i;
        msg[1] = i >> 8;
        CHECK(haskellsecp256k1_v0_1_0_schnorrsig_sign_custom(data->ctx, sig, msg, MSGLEN, data->keypairs[i], NULL));
    }
}
...
static void bench_schnorrsig_verify(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(haskellsecp256k1_v0_1_0_xonly_pubkey_parse(data->ctx, &pk, data->pk[i]) == 1);
        CHECK(haskellsecp256k1_v0_1_0_schnorrsig_verify(data->ctx, data->sigs[i], data->msgs[i], MSGLEN, &pk));
    }
}
...
static void run_schnorrsig_bench(int iters, int argc, char** argv) {
    ...
    data.keypairs = (const haskellsecp256k1_v0_1_0_keypair **)malloc(iters * sizeof(haskellsecp256k1_v0_1_0_keypair *));
    ...
    for (i = 0; i < iters; i++) {
        ...
        haskellsecp256k1_v0_1_0_keypair *keypair = (haskellsecp256k1_v0_1_0_keypair *)malloc(sizeof(*keypair));
        ...
        CHECK(haskellsecp256k1_v0_1_0_keypair_create(data.ctx, keypair, sk));
        CHECK(haskellsecp256k1_v0_1_0_schnorrsig_sign_custom(data.ctx, sig, msg, MSGLEN, keypair, NULL));
        CHECK(haskellsecp256k1_v0_1_0_keypair_xonly_pub(data.ctx, &pk, NULL, keypair));
        CHECK(haskellsecp256k1_v0_1_0_xonly_pubkey_serialize(data.ctx, pk_char, &pk) == 1);
    }
